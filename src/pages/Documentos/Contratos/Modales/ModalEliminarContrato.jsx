  ...
  const mostrarNotificacionExito = () => {
    if (typeof window !== 'undefined' && window.Swal) {
      window.Swal.fire({
        ...
        text: 'El contrato ha sido eliminado correctamente del sistema',
        icon: 'success',
        iconHtml: '✓',
        iconColor: '#28a745',
  ...
  const manejarEliminar = async () => {
    try {
      ...
      if (typeof window !== 'undefined' && window.Swal) {
        window.Swal.fire({
          title: 'Error',
          text: 'Hubo un problema al eliminar el contrato. Por favor, intenta nuevamente.',
          icon: 'error',
        ...
        alert('Error al eliminar el contrato. Intenta nuevamente.');
      }
    }
  };
  ...
  return (
    <div className="modal-overlay-simple" onClick={manejarCancelar}>
      <div className="modal-container-simple" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 className="modal-title-simple">¿Eliminar Contrato?</h3>

        <p className="modal-message-simple">
          Esta acción eliminará a <strong>{contrato.nombre}</strong> del sistema.
        </p>
