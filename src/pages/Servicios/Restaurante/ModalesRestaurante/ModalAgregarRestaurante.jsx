...
import './ModalAgregarRestaurante.css';
...
const ModalAgregarRestaurante = ({ onGuardar, onCerrar, proveedores = [] }) => {
      ...
      await Swal.fire({
        ...
        html: `
          <div style="font-size: 1rem; margin-top: 10px; color: #64748b;">
            <p>Hubo un problema al guardar el servicio de restaurante.</p>
  ...
  const MensajeError = ({ nombreCampo }) => {
    ...
    return <span className="modal-resto-error-mensaje">{error}</span>;
  };

  const renderSeccionGenerales = () => (
    <div className="modal-resto-form-grid">
      <div className="modal-resto-form-group">
        <label htmlFor="nombre_servicio">
          Nombre del Servicio <span className="modal-resto-required">*</span>
        </label>
        <input
          type="text"
          id="nombre_servicio"
          ...
          className={errores.nombre_servicio ? 'modal-resto-input-error' : ''}
      ...
      <div className="modal-resto-form-group">
        <label htmlFor="tipo_servicio">
          Tipo de Servicio <span className="modal-resto-required">*</span>
        </label>
        <select
          id="tipo_servicio"
          name="tipo_servicio"
          ...
          onChange={handleChange}
          className={errores.tipo_servicio ? 'modal-resto-input-error' : ''}
        >
      ...
      <div className="modal-resto-form-group">
        <label htmlFor="categoria">
          Categoría <span className="modal-resto-required">*</span>
        </label>
        <select
          id="categoria"
          name="categoria"
          ...
          onChange={handleChange}
          className={errores.categoria ? 'modal-resto-input-error' : ''}
        >
          <option value="">Seleccionar...</option>
          {categorias.map(cat => (
      ...
      <div className="modal-resto-form-group">
        ...
        <input
          ...
          onChange={handleChange}
          className={errores.capacidad ? 'modal-resto-input-error' : ''}
          placeholder="50"
          min="1"
          max="500"
        />
      ...
      <div className="modal-resto-form-group modal-resto-form-group-full">
        <label htmlFor="descripcion_servicio">
          Descripción del Servicio <span className="modal-resto-required">*</span>
        </label>
        <textarea
          ...
          onChange={handleChange}
          className={errores.descripcion_servicio ? 'modal-resto-input-error' : ''}
    ...
    <div className="modal-resto-form-grid">
      <div className="modal-resto-form-group">
        <label htmlFor="tipo_paquete">
          Tipo de Paquete <span className="modal-resto-required">*</span>
        </label>
        <select
          id="tipo_paquete"
          name="tipo_paquete"
          ...
          onChange={handleChange}
          className={errores.tipo_paquete ? 'modal-resto-input-error' : ''}
        >
          <option value="">Seleccionar...</option>
      ...
      <div className="modal-resto-form-group">
        <label htmlFor="precio_base">
          Precio Base <span className="modal-resto-required">*</span>
        </label>
        <input
          type="number"
          id="precio_base"
          ...
          onChange={handleChange}
          className={errores.precio_base ? 'modal-resto-input-error' : ''}
          placeholder="350.00"
          step="0.01"
          min="0"
        />
      ...
      <div className="modal-resto-form-group">
        <label htmlFor="moneda">
          Moneda <span className="modal-resto-required">*</span>
        </label>
        <select
          id="moneda"
          name="moneda"
    ...
    <div className="modal-resto-form-grid">
      ...
      <div className="modal-resto-form-group">
        <label htmlFor="ubicacion_restaurante">
          Ubicación <span className="modal-resto-required">*</span>
        </label>
        <input
          type="text"
          ...
          onChange={handleChange}
          className={errores.ubicacion_restaurante ? 'modal-resto-input-error' : ''}
      ...
      <div className="modal-resto-form-group">
        <label className="modal-resto-checkbox-label">
          <input
            type="checkbox"
            name="disponibilidad"
  ...
  const renderSeccionDocumentos = () => (
    <div className="modal-resto-form-grid-documentos">
      <div className="modal-resto-form-group-file">
        ...
        {formData.foto_servicio && (
          <span className="modal-resto-file-name">{formData.foto_servicio.name}</span>
        )}
      </div>
    </div>
  );
    ...
    <div className="modal-resto-overlay" onClick={onCerrar}>
      <div className="modal-resto-contenido modal-resto-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-resto-tabs">
          ...
          <button
            className={`modal-resto-tab-button ${seccionActiva === 'paquete' ? 'active' : ''}`}
          ...
          <button
            className={`modal-resto-tab-button ${seccionActiva === 'ubicacion' ? 'active' : ''}`}
          ...
          <button
            className={`modal-resto-tab-button ${seccionActiva === 'documentos' ? 'active' : ''}`}
        ...
        <div className="modal-resto-footer">
          ...
          <div className="modal-resto-botones-derecha">
            <button
              type="button"
              className={`modal-resto-btn-guardar ${guardando ? 'loading' : ''}`}
              disabled={guardando}
